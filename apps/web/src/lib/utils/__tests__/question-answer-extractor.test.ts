import { extractAnswersFromLatex, prepareAnswersForDatabase, prepareAnswersForForm } from '../question-answer-extractor';
...
describe('Question Answer Extractor', () => {
  ...
  describe('prepareAnswersForDatabase', () => {
    it('should prepare answers and correctAnswer for multiple-choice questions', () => {
      // Mock kết quả trả về từ extractFromLatex
      (extractFromLatex as jest.Mock).mockReturnValue({
        type: 'multiple-choice',
        answers: [
      ...
      const result = prepareAnswersForDatabase('\\begin{ex}\\choice{Đáp án 1}{Đáp án 2}{\\True Đáp án 3}{Đáp án 4}\\end{ex}');

      expect(result).toEqual({
        type: 'MC',
        answers: JSON.stringify(['Đáp án 1', 'Đáp án 2', 'Đáp án 3', 'Đáp án 4']),
        correctAnswer: JSON.stringify('Đáp án 3')
      });
    });

    it('should prepare answers and correctAnswer for true-false questions', () => {
      // Mock kết quả trả về từ extractFromLatex
      (extractFromLatex as jest.Mock).mockReturnValue({
        type: 'true-false',
        answers: [
      ...
      const result = prepareAnswersForDatabase('\\begin{ex}\\choiceTF{\\True Đáp án 1}{Đáp án 2}{\\True Đáp án 3}{Đáp án 4}\\end{ex}');

      expect(result).toEqual({
        type: 'TF',
        answers: JSON.stringify(['Đáp án 1', 'Đáp án 2', 'Đáp án 3', 'Đáp án 4']),
        correctAnswer: JSON.stringify(['Đáp án 1', 'Đáp án 3'])
      });
    });

    it('should prepare correctAnswer for short-answer questions', () => {
      // Mock kết quả trả về từ extractFromLatex
      (extractFromLatex as jest.Mock).mockReturnValue({
        type: 'short-answer',
        answers: [],
      ...
      const result = prepareAnswersForDatabase('\\begin{ex}\\shortans{Đáp án đúng}\\end{ex}');

      expect(result).toEqual({
        type: 'SA',
        answers: null,
        correctAnswer: JSON.stringify('Đáp án đúng')
      });
    });
    ...
    it('should return null for essay questions', () => {
      ...
      const result = prepareAnswersForDatabase('\\begin{ex}Câu hỏi tự luận\\end{ex}');
  ...
  describe('prepareAnswersForForm', () => {
    it('should prepare answers and correctAnswer for multiple-choice questions', () => {
      const result = prepareAnswersForForm({
        type: 'MC',
        answers: JSON.stringify(['Đáp án 1', 'Đáp án 2', 'Đáp án 3', 'Đáp án 4']),
        correctAnswer: JSON.stringify('Đáp án 3')
      });
